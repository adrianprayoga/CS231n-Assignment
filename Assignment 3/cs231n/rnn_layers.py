...
def rnn_step_backward(dnext_h, cache):
    ...
    #dtanh = np.square(1/np.cosh(temp))*dnext_h # Shape N x H
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        if t==0:
            ...
            ht_1 = h[:, t-1, :]
        cache_t = (xt, ht_1, Wx, Wh, b, ht)
        dx_, dprev_h, dWx_, dWh_, db_ = rnn_step_backward(dnext_h+dprev_h, cache_t)
        dWx += dWx_
        dWh += dWh_
        db += db_
        dx[:, t, :] = dx_
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    N, T, V = x.shape #V is indicates the word location in the vocabulary list
