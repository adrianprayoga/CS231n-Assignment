...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == "rnn":
            h, nn_cache = rnn_forward(x, h0, Wx, Wh, b)
        else:
            pass
        ...
        if self.cell_type == "rnn":
            dout, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dout, nn_cache)
        else:
            pass
    ...
    def sample(self, features, max_length=30):
        ...
        #step 2
        caption_in = captions[:,0]
        #caption_in = self._start * np.ones((N, 1), dtype=np.int32)
        prev_h = h0

        for t in range(max_length):
            x, _ = word_embedding_forward(caption_in, W_embed)
            if self.cell_type == "rnn":
